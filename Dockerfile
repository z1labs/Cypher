...
WORKDIR /workspace
RUN apt-get update && apt-get install -y curl build-essential cmake make clang wabt
RUN curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y

ENV PATH="/root/.cargo/bin:${PATH}"
RUN rustup install nightly-2024-03-11
RUN rustup default nightly-2024-03-11
...
FROM rust-base AS prover-builder
WORKDIR /workspace
RUN export DEBIAN_FRONTEND=noninteractive && \
    apt-get update && \
...
FROM rust-base AS tfhe-builder
WORKDIR /workspace
RUN export DEBIAN_FRONTEND=noninteractive && \
    apt-get update && \
    apt-get install -y make wget gpg software-properties-common zlib1g-dev libstdc++-12-dev wabt
COPY fhevm-go/tfhe-rs/ fhevm-go/tfhe-rs/
WORKDIR /workspace/fhevm-go/tfhe-rs
RUN RUSTFLAGS="-C target-cpu=native -C relocation-model=pic" cargo build --release --features=x86_64-unix,high-level-c-api -p tfhe
RUN make symlink_c_libs_without_fingerprint

FROM rust-base AS tfhe-bmaker
WORKDIR /workspace
RUN export DEBIAN_FRONTEND=noninteractive && \
    apt-get update && \
    apt-get install -y make wget gpg software-properties-common zlib1g-dev libstdc++-12-dev wabt
COPY fhevm-go/ fhevm-go/
WORKDIR /workspace/fhevm-go/
RUN make build
...
RUN export DEBIAN_FRONTEND=noninteractive && \
    apt-get update && \
    apt-get install -y wabt binutils binutils-gold gcc g++

COPY go.mod go.sum ./
COPY go-ethereum/go.mod go-ethereum/go.sum go-ethereum/
COPY fastcache/go.mod fastcache/go.sum fastcache/
COPY fhevm-go/go.mod fhevm-go/go.sum fhevm-go/
RUN go mod download
COPY . ./
...
COPY --from=tfhe-builder /workspace/fhevm-go/tfhe-rs/target/release/deps/libtfhe.a /usr/local/lib/libtfhe.a
COPY --from=tfhe-builder /workspace/fhevm-go/tfhe-rs/target/release/deps/libtfhe_c_api_dynamic_buffer.a /usr/local/lib/libtfhe_c_api_dynamic_buffer.a

RUN ldconfig

COPY . ./
RUN go mod download
RUN NITRO_BUILD_IGNORE_TIMESTAMPS=1 make build VERBOSE=1
...
ENV TFHE_EXECUTOR_CONTRACT_ADDRESS="0x047aF4f7F615F795baB5D367a7a1050D29B80690"
...
# Ensure that all necessary environment variables are accessible
RUN ./validate-wasm-module-root.sh /home/user/target/machines /usr/local/bin/prover

USER root

ENV FHEVM_GO_KEYS_DIR=/home/user/.fhevm-go/keys

# Create the necessary directory structure in the container
RUN mkdir -p $FHEVM_GO_KEYS_DIR

# Copy the keys directory from the build context (where Dockerfile is located) into the container
COPY keys/ $FHEVM_GO_KEYS_DIR/
